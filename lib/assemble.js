...
function requireBankSelectionArg(statement, node) {
  ...
  if (arg.type !== 'bankSelection') {
    throwAssemblyError('.bank expects a bank selection.', node);
  }
  return arg;
}
...
function expandBankSelection(selection, config, symbols, node) {
  const banks = [];
  const seen = new Set();
  ...
  for (const selector of selection.selectors) {
    for (const bank of expandBankSelector(selector, config, symbols, node)) {
      ...
      if (seen.has(bank)) {
        throwAssemblyError(`Bank ${bank} appears more than once in .bank selection.`, node);
      }
      seen.add(bank);
      banks.push(bank);
    }
  ...
  if (banks.length === 0) {
    throwAssemblyError('.bank selection cannot be empty.', node);
  }
  return banks;
}
...
function collectLayout(program, config, initialSymbols = new Map()) {
  ...
  for (let lineIndex = 0; lineIndex < program.body.length; lineIndex++) {
    ...
    if (node.statement?.type === 'directive' && node.statement.normalized === 'bank') {
      activeBanks = expandBankSelection(requireBankSelectionArg(node.statement, node), config, symbols, node);
