...
function mapperNameFromArg(arg, node) {
  ...
  if (arg.type === 'number' || arg.type === 'builtinConstant') {
    return arg.value;
  }
...
function parameterValueIfKnown(parameter, symbols) {
  if (parameter.type === 'number' || parameter.type === 'builtinConstant') return parameter.value;
...
function resolveImmediateParameter(parameter, node) {
  if (parameter.type !== 'number' && parameter.type !== 'builtinConstant') {
